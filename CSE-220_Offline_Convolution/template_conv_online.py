...
Instructions:
- Copy (or import) your completed DiscreteSignal and LTI_System classes from Offline 1.
...
def first_difference(sig: DiscreteSignal) -> DiscreteSignal:
    ...
    Must use DiscreteSignal.shift/add/multiply.
    """
    # TODO
    del_sig=sig.add(sig.shift(1).multiply(-1))
    return del_sig
...
def impulse_from_step_response(step_response: DiscreteSignal) -> DiscreteSignal:
    ...
    Must use only DiscreteSignal operations.
    """
    # TODO
...
if __name__ == "__main__":
    ...
    sys_h = impulse_from_step_response(s)
    y_h = LTISystem(sys_h).output(x)
