...
def signal_from_samples(start_time, end_time, samples):
    ...
    for idx,val in samples.items():
        signal.set_value_at_time(idx,val)
    return signal
...
def impulse_identity():
    return DiscreteSignal.unit_impulse(0)
...
def max_absolute_difference(first_signal, second_signal):
    ...
    for i in range(start,end+1):
        dif=max(dif,np.abs(first_signal.get_value_at_time(i)-second_signal.get_value_at_time(i)))
    return dif
...
def plot_color_blocks(signal, title, ax):
    ...
    for boundary in range(len(signal) + 1):
